...
main = do
  args <- getArgs
  let popts = zip args (map parseOption args)
      (cppopt, cppfiles) = partition (isJust . snd) popts
  let (hsrc, inclfile) = 
        case (map fst cppfiles) of
          [] -> (hGetContents stdin, "<stdin>")
          ["-"] -> (hGetContents stdin, "<stdin>")
          (a:_) -> (openFile a ReadMode >>= hGetContents, a)
  let baseopt = [CpphsPragma, CpphsMacro ("boolean", "Bool")]
  hsrcpr <- hsrc >>= runCpphs (map (fromJust . snd) cppopt ++ baseopt) inclfile
...
mod2mod st md@(I.Module _ moddefs) = 
  H.HsModule nullLoc (H.Module modid') (Just []) imps decls where
    ...
    imps = map mkModImport (map H.Module (["Control.Monad" ,"UnsafeJS"] ++ imp st))
    ...
    tag -> [mktsig, mkimpl] where
      ...
      mktsig = 
        let monadtv = mkTIdent "mn"
            ...
            monadspec = (mkUIdent "JSpecM",[monadtv])
            tpsig = mkTsig parms (H.HsTyApp monadtv (mkTIdent (typeFor iid)))
            retts = H.HsQualType (monadspec : monadctx : actx : []) tpsig in
  ...
  concat $ map mkattr $ collectAttrs intf where
    ...
    stsig iid iat tat =
      let monadtv = mkTIdent "mn"
          ...
          retts = H.HsQualType (monadspec : monadctx : contxt) tpsig in
    ...
    gtsig iid iat tat = 
      let monadtv = mkTIdent "mn"
          ...
          retts = H.HsQualType (monadspec : monadctx : thisctx : ctxRet tat) tpsig in
  ...
  concat $ map mkmeth $ collectOps intf where
    ...
    tsig op@(I.Operation (I.FunId _ _ parm) optype _ _) = 
      let monadtv = mkTIdent "mn"
          ...
          retts = H.HsQualType (monadspec : monadctx : thisctx : contxt) tpsig in
